<?php
class Contactu extends CI_Model
{
...
//		$this->db->from('orders');
	...
	function get_order_product($order_id , $completed)
	{
		...
		foreach($results->result() as $res)
		{
			$query = "SELECT * FROM epos_product WHERE prod_id='".$res->prod_id."'";
			$res_prod = $this->db->query($query)->row();
			...
			if($completed == 0)
			{
				...
				$data .= $res_prod->prod_code;
				$data .= "</td><td style='width:42%;'>";
				$data .= $res_prod->prod_desc;
				$data .= "</td><td style='width:20%;'>";
				$data .= $res_prod->prod_pack_desc;
				$data .= "</td><td style='width:5%;'>";
				$data .= $res_prod->prod_uos;

				$data .= "</td><td style='width:10%;'>";
				$data .= $res_prod->prod_sell;
			...
			else
			{
				...
				$data .= $res->quantity;
				$data .= "</td><td style='width:20%;'>";
				$data .= "<div class='tiny_long_long_button' style='float: left;' onmouseover='this.className=\"tiny_long_long_button_over\"' onmouseout='this.className=\"tiny_long_long_button\"' onclick='set_qty(this , $res_prod->prod_id);'><span>".$this->lang->line('pastorders_add_to_current_trolley')."</span></div>";
				$data .= "</td></tr>";
