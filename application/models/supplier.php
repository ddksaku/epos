...
class Supplier extends Person
{
	...
	*/
	function get_info($supplier_id)
	{
		$this->db->from('employees');
		$this->db->join('people', 'people.person_id = employees.person_id');
		$this->db->where('employees.person_id',$supplier_id);
		$query = $this->db->get();
	...
	*/
	function get_multiple_info($suppliers_ids)
	{
		$this->db->from('employees');
		$this->db->join('people', 'people.person_id = employees.person_id');
		$this->db->where_in('employees.person_id',$suppliers_ids);
		$this->db->order_by("first_name", "asc");
		return $this->db->get();
	}
	...
	function save(&$person_data, &$supplier_data,$supplier_id=false)
	{
		...
		if(parent::save($person_data,$supplier_id))
		{
			if (!$supplier_id or !$this->exists($supplier_id))
			{
				...
				$success = $this->db->insert('employees',$supplier_data);
			}
			else
			{
				$this->db->where('person_id', $supplier_id);
				$success = $this->db->update('employees',$supplier_data);
			}
...
//		return $this->db->update('suppliers', array('deleted' => 1));
	...
	*/
	function get_search_suggestions($search,$limit=25)
	{
		...
		CONCAT(`first_name`,' ',`last_name`) LIKE '%".$this->db->escape_like_str($search)."%') and deleted=0 and account_category=1");
		$this->db->order_by("first_name", "asc");
		$by_name = $this->db->get();
